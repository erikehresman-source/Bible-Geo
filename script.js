...
const mapMediaRow=document.getElementById("map-media-row");
...
// ===== State =====
let DATA=[];     // loaded from questions.json
let filtered=[];
let order=[];
let currentIndex=0;
let mode="easy";
...
function setupMapTarget(coord){
  mapEl.innerHTML="";
  if(!coord){ mapStatusEl.textContent="No map target for this clue."; return; }
...
function placePin(x,y){
  const old=mapEl.querySelector(".pin"); if(old) old.remove();
...
// ===== Game flow =====
function startGame(){
  if(!DATA || !DATA.length){ alert("Questions not loaded yet. Please try again (host via GitHub Pages)."); return; }
  ...
  startBtn.disabled=true; restartBtn.disabled=false; themeEl.disabled=true; difficultyEl.disabled=true; mapToggleEl.disabled=true;
...
function resetUI(){
  startBtn.disabled=false; restartBtn.disabled=true; themeEl.disabled=false; difficultyEl.disabled=false; mapToggleEl.disabled=false;
  ...
  mapMediaRow.classList.add("hidden"); mapEl.innerHTML=""; mapStatusEl.textContent=""; mediaImg.src=""; mediaImg.alt=""; mediaCaption.textContent="";
...
function showCurrent(){
  ...
  // Prepare map & media for this item but keep row hidden until correct
  mapMediaRow.classList.add("hidden");
  setupMapTarget(item.map);
  loadMedia(null); // clear
}

function renderChoices(correct){
  const answers=uniqueAnswers(filtered);
  const pool=answers.filter(a=>normalize(a)!==normalize(correct));
...
function correctAnswerFlow(){
  ...
  // reveal map + image immediately with correct point marked
  const item=filtered[order[currentIndex]];
  showMapAndMedia(item);

  if(mapToggleEl.checked){
    // still allow the user to click for bonus even though the target is visible
    awaitingMapClick=true;
    mapStatusEl.textContent="Bonus: click the map near this location for +3.";
    // show target/ring already; pulse for visibility
  ...
  // no map bonus: brief pause to let them see, then continue
  setTimeout(nextOrFinish, 800);
}

function showMapAndMedia(item){
  mapMediaRow.classList.remove("hidden");
  // Reveal target & pulse at the known location
  const dot=document.getElementById("target-dot");
  const ring=document.getElementById("target-ring");
  if(dot && ring){ dot.style.display="block"; ring.style.display="block"; pulseAt(item.map.x, item.map.y); }
  // Auto-place a pin exactly on the location for clarity
  if(item.map){ placePin(item.map.x, item.map.y); }
