...
function setupMapTarget(coord){
  // Do NOT clear pins; only reset the target elements
  removeTarget();
...
function showCurrent(){
  ...
  // Always keep the map visible; only update the target for the current item
  setupMapTarget(item.map);
  loadMedia(null); // hide media until answered
}
...
function correctAnswerFlow(){
  ...
  // reveal current target + drop a persistent pin
  const item=filtered[order[currentIndex]];
